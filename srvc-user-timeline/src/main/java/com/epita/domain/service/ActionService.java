...
import com.epita.data.model.ActionModel;
...
public class ActionService {
    ...
    public void storeAction(ActionModel action) throws IOException {
        ...
        GetResponse<ActionsModel> actionsModel = elasticsearchClient.get(getRequest -> getRequest.index("actions").id(action.userId), ActionsModel.class);
        ...
        if (actionsModel.source() != null) {
            ...
            elasticsearchClient.index(indexRequest -> indexRequest.index("actions").id(action.userId).document(actions));
        } else {
            System.out.println("Action doesn't exist");
            // if it doesn't exist, create a new document
            ActionsModel actions = new ActionsModel(action);
            IndexRequest<ActionsModel> request = IndexRequest.of(b -> b.index("actions").id(action.userId).document(actions));
